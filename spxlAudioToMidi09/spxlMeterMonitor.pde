 ...
 * - displayInfo() for value changes / messages (especially helpful for MIDI control)
 */
...
public class MeterMonitor
{
  ...
  // Number of display updates to show meter information
  protected final int NUM_DISPLAY_INFO_FRAMES = 30;
  protected final int NUM_DISPLAY_INFO_FADE_FRAMES = 8;
  
  // Counter for meter information display
  protected int displayInfoCounter;
  
  // Meter information display string (eg recently set output range)
  protected String displayInfoStr;
                      ...
                      MidiManager midiManager, int controllerNumber)
  {
    ...
    p5.registerMethod("mouseEvent", this); // (v06) Processing 2.0 changes
  }
  ...
  public void draw()
  {
    ...
    else
    {
      ...
      if (displayInfoCounter > 0)
      {
        
        int alpha = 255;
        if (displayInfoCounter <= NUM_DISPLAY_INFO_FADE_FRAMES)
        {
          alpha = (int) p5.map(displayInfoCounter, 1, NUM_DISPLAY_INFO_FADE_FRAMES, 5, 255);
        }
  ...
  public void mouseEvent(MouseEvent event)
  {
    ...
    if ((pointRel.y >= 0) && (pointRel.y < h))
    {
      ...
      {
        mouseOver = true;
        mouseValue = (int)(0.5f + p5.map(pointRel.x, 0, w - 1, MIN_CONTROLLER_VALUE, MAX_CONTROLLER_VALUE));
      }
  ...
  public void setMeterLink(int meterIndex)
  {
    ...
    displayInfo("Meter: " + (meterIndex + 1));
    update();
  }
  ...
  {
    if (value < MIN_CONTROLLER_VALUE || value > MAX_CONTROLLER_VALUE) return;
    
    controllerMinValue = value;
    displayRange();
    update();
  }
  ...
  {
    displayInfo(dispValue(controllerMinValue) + "-" + dispValue(controllerMaxValue));
  }
  
  public void displayInfo(String message)
  {
    displayInfoStr = message;
    displayInfoCounter = NUM_DISPLAY_INFO_FRAMES;
  }
}

 ...
 *
 * [[] Decrease expected CC numbers (by 20) and note pitches (by 24) for MIDI input
 * []] Increase expected CC numbers (by 20) and note pitches (by 24) for MIDI input
 *
...
void setup()
{
  ...
  midiManager.setPlugSetup(this, "plugSetup");
}
...
void plugController(promidi.Controller controller)
{
  ...
  println("ccNumber: " + ccNumber + ", ccValue: " + ccValue);
  ...
  {
    ccValue = 0;
    println("  Mapped ccValue to: " + ccValue);
  }
...
void plugNote(promidi.Note note)
{
  ...
  println("velocity: " + velocity + ", pitch: " + pitch);
